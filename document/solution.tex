...
\begin{warning}
    大象无法被弓箭射击到 -> 必须勾选网格体“碰撞”“生成重叠事件”（因为大象的体积比较大，包裹住了胶囊体），并选择OverlapAllDynamic预设
\end{warning}
...
\begin{itemize}
    \item  新建可供骑乘的角色蓝图（狮子），并不是之前的动物蓝图的子类。
    \item  添加球体碰撞体作为坐骑触发器，并添加BPI\_Mount用作交互。
   
    \item  给狮子添加动画(F8.7)，平时趴在地上，看到玩家才起来，并面向玩家（F8.6），玩家按下F键后骑上去。
    \item  当骑上狮子以后，玩家控制器的控制权转移到狮子上，并且将玩家角色附着在狮子上。因此需要连接一个弹簧臂摄像机，勾选“使用pawn控制旋转”，禁用“使用控制器旋转yaw”，并勾选移动组件“将旋转朝向运动”
    \item  骑狮子以后，首先要隐藏起我们原来的第三人称角色。接着在狮子上放置替身模型,并调整动画腿部打开一点。在狮子蓝图中添加控制器逻辑（从玩家控制器中粘贴）
    \item  给狮子的运动动画做了二维处理，允许传递向右向量从而实现倾斜运动。
    \item  当在狮子状态时，再次使用交互键可以下狮子。（使用“获取类的actor节点找回角色”）下狮子时我们实际的角色实际上还在原地，因此要对其移动，我们在狮子中添加一个不可见无碰撞的骨骼作为参考变换。
    \item 
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{狮子朝向.png}
\caption{狮子朝向玩家}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{狮子ABP.png}
\caption{狮子ABP}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{上下狮子.png}
\caption{上下狮子}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{宠物跟随.png}
\caption{宠物跟随}
\end{figure}
...
攻击：
\begin{itemize}
    \item 角色动画本身在空中有一个基准高度，导致mesh飞出胶囊体，而且摄像机不能跟上 -> 基础高度在pelvis，在动画首帧调整位置新建关键帧
    \item 仿照连招系统，做出空中连招系统。区别在于空中连招的每次攻击都需要向上弹射角色，且空中连招至多四段，一旦结束，不允许再攻击。
    \item 着陆重新判定可攻击状态
\end{itemize}

冲刺:
\begin{itemize}
    \item 目前，为了防止角色多次按翻滚键导致的重复翻滚，设置了Dodgging变量，当蒙太奇播放完成后，才会重置。然而这样存在问题，即如果蒙太奇没完全播放完成，
          被其他蒙太奇打断，那么我们将永远不能再翻滚。 -> delay动画播放时间后重置Dodging变量
    \item 区分角色是否在空中，播放两种动画冲刺
\end{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{空中连招.png}
\caption{空中连招}
\end{figure}
...
\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{着陆重置.png}
\caption{着陆重置攻击}
\end{figure}
...
\begin{itemize}
    \item 目前角色攻击朝向正前方，且只有在每一段攻击动画完成后，才可以改变攻击方向，导致攻击手感很僵硬
    \item 在调用攻击入口后，旋转玩家朝向视角所指的方向（将实际旋转改为控制旋转）
    \item 添加过渡（由于这是一个组件而不是数值，因此我们不能使用时间轴，而要用RInterpToRotation），并且只考虑z轴旋转
    \item RInterpTo需要在每帧调用。这里我们建立一个变量用于管理是否改变攻击锁定，每次进入攻击令其为true，dalay短时间后改为false
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=1.0\textwidth]{调整角色攻击时旋转.png}
\caption{调整角色攻击时旋转}
\end{figure}
...
玩家移动：
\begin{itemize}
    \item  正常移动，由speed驱动locomotion，正常速度为500。当玩家转向时，通过叠加动画让角色倾斜
    \item  跳跃（空格），跳跃过程中允许冲刺或者空中连招
    \item  蹲伏（左ctrl）：胶囊体半高减小，视角拉长，最大速度变为350
    \item  翻越（左shift）： 允许翻越不超过玩家高度，且不太厚的障碍物，此过程需要计算三个位置：障碍物起点，障碍物终点，角色落点，并通过motion warp设置角色位置。该运动通过“飞行”模式实现。
           若物体过高（球形处产生overlap）或过厚（超过检测的最大范围，从而没计算出落点），则不允许翻越。
    \item  冲刺（按住左alt）：消耗耐力，将玩家最大移动速度设为1000，并增大FOV，调快奔跑动画播放速率。耐力消耗完毕后，没有冲刺才会缓慢恢复（在tick中检查是否在冲刺）
    \item  闪避（X）：消耗耐力大范围范围，区分普通闪避和空中闪避
    \item  攀爬（Q）：使用飞行模式，附着在表面。将向上输入（WS）改为上下移动了。攀爬时持续发出射线检测是否命中墙体，若命中，则依据命中点的法线修正玩家角度
    \item  抓边攀爬：攀爬至表面或者跳跃时再按跳跃，在角色头顶发出水平射线检测，一旦没命中说明头部位置为空，再发出若干条垂直射线，命中位置即攀爬边缘。
    \item  游泳：水体中时，玩家会触发游泳，只接受ws输入
\end{itemize}

属性系统：
\begin{itemize}
    \item 在BPC中实现，拥有血量，耐力，等级以及相关增加减少逻辑。
    \item 玩家拥有HUD，显示血量，耐力，经验，剩余箭矢数量。
    \item 武器的拾取通过针对object检测（动态场景）实现，装备通过特定的插槽实现。我们在人物模型上本身就预留了可装备的静态网格体，并且设置了骨骼上的槽位。显示通过可视性实现。
    \item 玩家受击会被击退，并且播放红边UI动画
    \item 武器使用结构体存储，访问则统过“数据表行处理”。
    \item 交互（F）：可以拾取武器与装备
    \item 背包（M）：装备已经拾取的武器
\end{itemize}
...
攻击系统：
\begin{itemize}
    \item  刺杀（F）：当玩家在敌人背面触碰到球体碰撞区域，则弹出UI提示框。若玩家按下刺杀，则自己播放刺杀蒙太奇，并且使用BPI通知受击者播放受击动画并直接杀死敌人（布娃娃）。
    在敌人蓝图中放了一个带有动画的无碰撞骨骼网格体用于绑定刺杀动画位置，并使用motion warp将玩家移动过去。
    \item  连招（左键）：当玩家装备了剑以后，可以按左键进行连招。连招在蒙太奇播放时使用Anim Notify通知来控制连招，在播放结尾时重置连招状态，通过攻击索引的转化来播放不同的攻击动画。
           在攻击时，通过AnimNotifyState通知（有开始和结束）启用或禁用球形检测从来产生伤害，球形检测的位置通过蓝图中预留的两个arrow组件控制。造成伤害会产生摄像机抖动，播放音效以及血液粒子效果。
    \item  目标锁定（Tab）对于damageable的敌人可以目标锁定，我们在tick中实时计算摄像机的方向，但是只有找到锁定目标的时候才会设置（使用get valid）。发射一个大型的球形检测，不能“按通道”而要针对Objects
    \item  扔石头（E）：生成一个可弹跳的可投掷物，石头在“组件命中时”报告一个噪点事件
    \item  射箭（鼠标右键）：若当前存在箭矢，且玩家装备了弓，则长按右键进入瞄准，同时锁定摄像机视角指向玩家正前方，同时生成准星UI，并轻微摄像机角度。松手播放射箭动画，在恰当的位置启用anim notify通知生成一个箭（可投掷物）飞向前方，并检测当组件开始重叠时。当玩家持弓时，不允许持剑，且播放持弓专属idle动画（按布尔混合姿势）。
    瞄准时获取玩家俯仰角，启用叠加动画调整瞄准偏移。玩家在瞄准或拉弓时移动，上半身播放瞄准动画，下半身播放奔跑动画，使用spine01为界限的分层混合
\end{itemize}

boss不允许被刺杀（给boss加上tag（boss），刺杀时检测不允许带有boss标签）


AI系统：
\begin{itemize}
    \item  dummy:最开始的测试假人，无任何AI逻辑，拥有血条，可以被刺杀以及攻击系统伤害。受击会扣血，播放受击动画并且产生击退，死亡会变成布娃娃
    \item  AI：在dummy的基础上增加了行为树与黑板，使用AIController控制（带有AI Perception，启用视力与听力，事件开始时启用行为树，目标感知更新时，获取刺激源，并设置黑板中的值） 。 
    AI如果看到玩家了，则使用服务判断与玩家的距离是否在范围内，并且追逐玩家。判断为真则攻击玩家。如果没看到，则如果收到了调查的噪点，去调查点调查，否则在随机半径内巡逻。
    AI攻击任务使用接口进行通知，并且使用玩家攻击系统中的逻辑实现攻击动画的播放与检测伤害，死亡后随机播放死亡动画，变成布娃娃，并且通过时间轴慢慢变淡消失。      
    \item  AI Boss: AI的子类，采用不同的行为树与黑板。相较于普通AI，boss体型更大，攻击播放更慢，有更高的基础伤害，拥有血条UI显示在玩家界面，不会被石子干扰以及刺杀。
    \item  AI Civilian：AI的子类，不允许被攻击（移除damageable标签），只会巡逻。
    \item  AI Animal：动物，使用简化版的AI逻辑（Pawn Sensing），没事就游荡，看见玩家就会过来攻击，仅会受到弓箭伤害，三次后死亡。
    \item  宠物：并非动物的子类，而是一类可以被控制的Pawn。玩家骑上去时由玩家控制，玩家下去时由AI控制器控制。
